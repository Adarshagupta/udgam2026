import { NextResponse } from "next/server";
import { z } from "zod";

import { getAuthSession } from "@/lib/auth";
import { createSport } from "@/lib/data";

const createSportSchema = z.object({
  name: z.string().trim().min(2),
  accent: z.string().trim().optional(),
  tagline: z.string().trim().optional(),
});

export const runtime = "nodejs";
export const dynamic = "force-dynamic";

export async function POST(request: Request) {
  const session = await getAuthSession();

  if (!session?.user?.id) {
    return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
  }

  const parsed = createSportSchema.safeParse(await request.json());

  if (!parsed.success) {
    return NextResponse.json({ error: "Invalid sport payload." }, { status: 400 });
  }

  try {
    const sport = await createSport(parsed.data);
    return NextResponse.json({ sport }, { status: 201 });
  } catch (error) {
    return NextResponse.json(
      {
        error: error instanceof Error ? error.message : "Could not create the sport.",
      },
      { status: 400 },
    );
  }
}
